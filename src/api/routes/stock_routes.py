...
class QuestionRequest(BaseModel):
    question: str
    context: str = (
        "You are a stock market enthusiast"  # Optional context (e.g., stock analysis)
    )
...
async def analyze_all_stock_data(ticker: str):
    ...
    try:
        ...
        logger.error(
            "Stock analysis failed - runtime error", ticker=ticker, error=str(e)
        )
...
async def post_process_question_with_llm(request: QuestionRequest):
    try:
        ...
        llm_response = await process_question_with_llm(
