...
class Settings(BaseSettings):
    ...
    MONGODB_DB_NAME: str = Field(
        default="tradeskee_db", description="MongoDB database name"
    )
    MONGODB_MIN_POOL_SIZE: int = Field(
        default=10, description="Minimum connection pool size"
    )
    MONGODB_MAX_POOL_SIZE: int = Field(
        default=100, description="Maximum connection pool size"
    )

    # Redis Settings (for future caching)
    REDIS_URL: Optional[str] = Field(
        default=None, description="Redis connection string (optional)"
    )
    ...
    ALPHA_VANTAGE_RATE_LIMIT: int = Field(default=5, description="API calls per minute")
    ...
    OLLAMA_MODEL: str = Field(
        default="llama2", description="Default Ollama model to use"
    )
